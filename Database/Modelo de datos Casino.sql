DROP DATABASE IF EXISTS PROYECTO_CASINO;
CREATE DATABASE PROYECTO_CASINO;
USE PROYECTO_CASINO;

-- TABLA USUARIO
CREATE TABLE USUARIO (
    DNI VARCHAR(9) NOT NULL,
    NOMBRE VARCHAR(100) NOT NULL,
    APELLIDO1 VARCHAR(100) NOT NULL,
    APELLIDO2 VARCHAR(100),
    FECHA_NACIMIENTO DATE NOT NULL,
    USER_NAME VARCHAR(100) NOT NULL UNIQUE,
    EMAIL VARCHAR(100) NOT NULL UNIQUE,
    USER_PASSWORD VARCHAR(255) NOT NULL,
    NUMERO_TELEFONO VARCHAR(15) NOT NULL,
    DINERO_USUARIO DOUBLE (10,2),
    ES_VIP BOOLEAN,
    NUMERO_TARJETA INT(16),
    TITULAR_TARJETA VARCHAR(100),
    FECHA_EXPIRACION DATE,
    CVC INT(3),
    CONSTRAINT PK_USUARIO PRIMARY KEY (DNI)
);

-- TABLA JUEGO
CREATE TABLE JUEGO (
	ID_JUEGO INT NOT NULL,
    NOMBRE_JUEGO VARCHAR(100) NOT NULL,
    CONSTRAINT PK_JUEGO PRIMARY KEY (ID_JUEGO)
);

-- TABLA CONVERSION
CREATE TABLE CONVERSION (
    ID_CONVERSION INT NOT NULL AUTO_INCREMENT,
    MULTIPLICADOR DECIMAL(10,2) NOT NULL,
    FECHA_CREACION TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ID_JUEGO INT NOT NULL,
    CONSTRAINT PK_CONVERSION PRIMARY KEY (ID_CONVERSION),
    CONSTRAINT FK_JUEGO_CONVERSION FOREIGN KEY (ID_JUEGO) REFERENCES JUEGO (ID_JUEGO)
);

-- TABLA HISTORICO
CREATE TABLE HISTORICO (
	ID_HISTORICO INT NOT NULL AUTO_INCREMENT,
	FECHA_LOG_HISTORICO DATE NOT NULL,
    ID_JUEGO INT NOT NULL,
	DNI VARCHAR(9) NOT NULL,
    CONSTRAINT PK_ID_HISTORICO PRIMARY KEY (ID_HISTORICO),
    CONSTRAINT FK_USUARIO_HISTORICO FOREIGN KEY (DNI) REFERENCES USUARIO (DNI),
    CONSTRAINT FK_JUEGO_HISTORICO FOREIGN KEY (ID_JUEGO) REFERENCES JUEGO (ID_JUEGO)
);

-- TABLA CAVEMANRUN (DINOSTEAK)
CREATE TABLE CAVEMANRUN (
	ID_LOG_CAVEMANRUN INT NOT NULL AUTO_INCREMENT,
    FECHA_LOG_CAVEMANRUN DATE NOT NULL,
    APUESTA INT NOT NULL,
    MULTIPLICADOR DOUBLE(8,2) NOT NULL,
    RESULTADO DOUBLE (10,2) NOT NULL,
    ID_HISTORICO INT NOT NULL,
    DNI VARCHAR(9) NOT NULL,
    ID_JUEGO INT NOT NULL,
    CONSTRAINT PK_CAVEMANRUN PRIMARY KEY (ID_LOG_CAVEMANRUN),
    CONSTRAINT FK_HISTORICO_CAVEMANRUN FOREIGN KEY (ID_HISTORICO) REFERENCES HISTORICO (ID_HISTORICO),
    CONSTRAINT FK_USUARIOHISTORICO_CAVEMANRUN FOREIGN KEY (DNI) REFERENCES USUARIO (DNI),
    CONSTRAINT FK_JUEGOHISTORICO_CAVEMANRUN FOREIGN KEY (ID_JUEGO) REFERENCES JUEGO (ID_JUEGO)
);

-- TABLA CAVERNSLOTS (TRAGAPERRAS)
CREATE TABLE CAVERNSLOTS (
	ID_LOG_CAVERNSLOTS INT NOT NULL AUTO_INCREMENT,
    FECHA_LOG_CAVERNSLOTS DATE NOT NULL,
    APUESTA INT NOT NULL,
    COMBINACION VARCHAR(100) NOT NULL,
    RESULTADO DOUBLE (10,2) NOT NULL,
    ID_HISTORICO INT NOT NULL,
    DNI VARCHAR(9) NOT NULL,
    ID_JUEGO INT NOT NULL,
    CONSTRAINT PK_CAVERNSLOTS PRIMARY KEY (ID_LOG_CAVERNSLOTS),
    CONSTRAINT FK_HISTORICO_CAVERNSLOTS FOREIGN KEY (ID_HISTORICO) REFERENCES HISTORICO (ID_HISTORICO),
    CONSTRAINT FK_USUARIOHISTORICO_CAVERNSLOTS FOREIGN KEY (DNI) REFERENCES USUARIO (DNI),
    CONSTRAINT FK_JUEGOHISTORICO_CAVERNSLOTS FOREIGN KEY (ID_JUEGO) REFERENCES JUEGO (ID_JUEGO)
);

SELECT * FROM USUARIO;
SELECT * FROM CONVERSION;
SELECT * FROM JUEGO;
SELECT * FROM HISTORICO;
SELECT * FROM CAVEMANRUN;
SELECT * FROM CAVERNSLOTS;

-- Insertar un usuario en la tabla USUARIO
INSERT INTO USUARIO (DNI, NOMBRE, APELLIDO1, APELLIDO2, FECHA_NACIMIENTO, USER_NAME, EMAIL, USER_PASSWORD, NUMERO_TELEFONO, DINERO_USUARIO, ES_VIP, NUMERO_TARJETA, TITULAR_TARJETA, FECHA_EXPIRACION, CVC)
VALUES ('12345678A', 'Javier', 'Rubio', 'Gigante', '2002-09-24', 'j4vier', 'javii.central.7@gmail.com', 'root1234', '600123456', 10.0, null, null, null, null, null);

-- Insertar un juego en la tabla JUEGO
INSERT INTO JUEGO (ID_JUEGO, NOMBRE_JUEGO)
VALUES (1, 'Caveman Run'),
       (2, 'Cavern Slots');

-- Insertar una conversión en la tabla CONVERSION
INSERT INTO CONVERSION (MULTIPLICADOR, ID_JUEGO) VALUES
(100.0, 1),
(50.0, 2);

-- Insertar un registro histórico en la tabla HISTORICO
INSERT INTO HISTORICO (ID_HISTORICO, FECHA_LOG_HISTORICO, ID_JUEGO, DNI)
VALUES (1, '2024-12-10', 1, '12345678A'),
       (2, '2024-12-11', 2, '12345678A');

-- Insertar un log en la tabla CAVEMANRUN
INSERT INTO CAVEMANRUN (ID_LOG_CAVEMANRUN, FECHA_LOG_CAVEMANRUN, APUESTA, MULTIPLICADOR, RESULTADO, ID_HISTORICO, DNI, ID_JUEGO)
VALUES (1, '2024-12-10', 50, 2.50, 125.00, 1, '12345678A', 1);

-- Insertar un log en la tabla CAVERNSLOTS
INSERT INTO CAVERNSLOTS (ID_LOG_CAVERNSLOTS, FECHA_LOG_CAVERNSLOTS, APUESTA, COMBINACION, RESULTADO, ID_HISTORICO, DNI, ID_JUEGO)
VALUES (1, '2024-12-11', 30, 'BAR-BAR-7', 90.00, 2, '12345678A', 2);
